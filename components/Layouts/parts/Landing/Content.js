import SmallCard from "@/components/Layouts/parts/SmallCard";
import MediumCard from "@/components/Layouts/parts/MediumCard";
import LargeCard from "@/components/Layouts/parts/LargeCard";
  ...
  return (
    <article className="max-w-7xl mx-auto px-8 sm:px-16">
      <section className="py-6">
        <h2 className="text-3xl font-bold pb-5 text-gray-700 dark:text-white select-none">
          Jelajahi Sekitar
        </h2>
      ...
      <section className="py-6">
        ...
        <div className="flex space-x-5 overflow-scroll scrollbar-hide p-3 -ml-3">
          {dataCarousel?.map(({ img, title }) => (
            <MediumCard key={`index-${title}`} img={img} title={title} />
          ))}
        </div>
      </section>
